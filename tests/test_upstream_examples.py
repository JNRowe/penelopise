# Examples extracted from https://github.com/todotxt/todo.txt which is GPL-3
# licensed
import datetime
...
def test_context_filter():
    result = [e for e in TEST_RESULTS if todo_txt.Context("phone") in e.contexts]
    assert len(result) == 2


def test_project_filter():
    result = [e for e in TEST_RESULTS if todo_txt.Project("GarageSale") in e.projects]
    assert len(result) == 2
...
def test_no_contexts():
    assert todo_txt.parse_data("Email SoAndSo at soandso@example.com").contexts == []
